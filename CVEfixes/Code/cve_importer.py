...
def assign_cwes_to_cves(df_cve: pd.DataFrame):
    ...
    df_cwes_class["cwe_id"] = add_cwe_class(
        df_cwes_class["problemtype_json"].tolist()
    )  # list of CWE-IDs' portion

    # exploding the multiple CWEs list of a CVE into multiple rows.
    df_cwes_class = (
        df_cwes_class.assign(cwe_id=df_cwes_class.cwe_id)
        .explode("cwe_id")
        .reset_index()[["cve_id", "cwe_id"]]
    )
    df_cwes_class = df_cwes_class.drop_duplicates(
        subset=["cve_id", "cwe_id"]
    ).reset_index(drop=True)
    df_cwes_class["cwe_id"] = df_cwes_class["cwe_id"].str.replace(
        "unknown", "NVD-CWE-noinfo"
    )

    no_ref_cwes = set(list(df_cwes_class.cwe_id)).difference(set(list(df_cwes.cwe_id)))
    if len(no_ref_cwes) > 0:
        cf.logger.debug(
            "List of CWEs from CVEs that are not associated to cwe table are as follows:"
        )
    ...
    assert df_cwes_class.set_index(
        ["cve_id", "cwe_id"]
    ).index.is_unique, "Primary keys are not unique in cwe_classification records!"
    assert set(list(df_cwes_class.cwe_id)).issubset(
        set(list(df_cwes.cwe_id))
    ...
    df_cwes_class.to_sql(
        name="cwe_classification", con=db.conn, if_exists="replace", index=False
    )
...
def import_cves():
    ...
    if db.table_exists("cve"):
        cf.logger.warning(
            "The cve table already exists, loading and continuing extraction..."
        )
